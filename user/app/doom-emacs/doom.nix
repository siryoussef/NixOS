...
in
{
  ...
  (setq user-full-name "''+settings.user.name+''") ; name
  (setq user-username "''+settings.user.username+''") ; username
  (setq user-mail-address "''+settings.user.email+''") ; email
  (setq user-home-directory "/home/''+settings.user.username+''") ; absolute path to home directory as string
  (setq user-default-roam-dir "''+settings.user.defaultRoamDir+''") ; absolute path to home directory as string
  (setq system-nix-profile "''+settings.system.profile+''") ; what profile am I using?
  (setq system-wm-type "''+settings.user.wmType+''") ; wayland or x11?
  (setq doom-font (font-spec :family "''+settings.user.font+''" :size 20)) ; import font
  (setq dotfiles-dir "''+settings.user.dotfilesDir+''") ; import location of dotfiles directory
 '';
}

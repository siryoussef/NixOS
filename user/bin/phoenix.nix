...
let
  ...
  myPhoenixScript = ''
      ...
      function sync_system {
        ...
        pushd ''+settings.user.dotfilesDir+'' &> /dev/null;
        if [ "$1" = "verbose" ]; then
      ...
      function update_flake {
        ...
        if [ "$1" = "verbose" ]; then
          ...
          then
            ...
            echo "Running rm package*.json";
            rm package*.json;
            echo "Returning to ''+settings.user.dotfilesDir+''"
            popd &> /dev/null;
          fi
          echo ""
        ...
        echo -e "Please run$GREEN git$NC diff HEAD flake.lock inside ''+settings.user.dotfilesDir+(" "+'' to see flake input changes";'')+
      ''}
      if [ "$1" = "sync" ]; then
        if [ "$#" = 1 ]; then
          sync_system;
